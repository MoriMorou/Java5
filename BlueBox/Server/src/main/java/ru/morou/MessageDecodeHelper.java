...
import ru.morou.exep.CheckSumException;
	...
	public StandardJsonQuery decodeTransference(ByteBuf frame, String pathClient) throws IOException {
		...
		if (code == FileTransferHelper.CODE_JSON) {
			...
			if (jsonQuery instanceof PathContainer) {
				...
				if (jsonQuery.getQueryType() == StandardJsonQuery.QueryType.SEND_FILE) {
			...
			if (jsonQuery.getQueryType() != StandardJsonQuery.QueryType.SEND_FILE || pathClient == CLIENT_COM_SENDFILE)
				out = jsonQuery;
