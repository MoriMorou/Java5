...
import ru.morou.logger.ClientConsoleLogAppender;
...
public class MainSceneController extends Controller implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		comp = new Comparator<LabelWithInfo>() {
			...
			public int compare(LabelWithInfo label1, LabelWithInfo label2) {
				if (label1.getFileType() == LabelWithInfo.FileType.DIRECTORY && label2.getFileType() == LabelWithInfo.FileType.FILE)
					return -1;
				else if (label1.getFileType() == LabelWithInfo.FileType.FILE && label2.getFileType() == LabelWithInfo.FileType.DIRECTORY)
					return 1;
				else
	...
	private void initializeBehaviorLabel(LabelWithInfo file, FilesSource filesSource) {
		...
		label.setOnMouseClicked(event -> {
			...
			if (event.getButton().equals(MouseButton.PRIMARY)) {
				...
				if (event.getClickCount() == 2 && file.getFileType() == LabelWithInfo.FileType.DIRECTORY) {
     ...
     * @param e текст для записи в консоль
     */
	...
	protected void finalize() throws Throwable {
		try {
			BlueBox.getInstance().disconnect();
		} finally {
			super.finalize();
		}
	}
